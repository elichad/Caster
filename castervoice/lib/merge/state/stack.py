...
class CasterState:
    ...
    def add(self, stack_item):
        if self.blocker is None:
            ...
            if (
                stack_item.rspec in self.blocker.get_triggers()
            ):  # let cancels go through
                self.unblock()
...
class ContextStack:
    ...
    def add(self, stack_item):
        ...
        stack_item_is_forward_seeker = (
            stack_item.type == StackItemSeeker.TYPE and stack_item.forward is not None
        )
        stack_item_is_continuer = ContextStack.is_asynchronous(stack_item.type)
        if (
            not stack_item.consumed
            and not stack_item_is_forward_seeker
            and not stack_item_is_continuer
        ):
    ...
    def get_incomplete_seekers(self):
        ...
        for i in range(0, len(self.list)):
            if not self.list[
                i
            ].complete:  # no need to check type because only forward seekers will be incomplete
